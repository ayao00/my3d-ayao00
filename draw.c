...
void add_sphere( struct matrix * edges,
                 ...
                 double r, int step ) {
  struct matrix * gen_sphere;
  gen_sphere = generate_sphere(cx, cy, cz, r, step);
  int i;
  for(i = 0; i < gen_sphere->lastcol; i++){
    add_edge(edges, gen_sphere->m[0][i],gen_sphere->m[1][i],gen_sphere->m[2][i],gen_sphere->m[0][i],gen_sphere->m[1][i],gen_sphere->m[2][i]);
      //gen_sphere->m[0][i+1],gen_sphere->m[1][i+1],gen_sphere->m[2][i+1]);
  }
}
...
void add_torus( struct matrix * edges,
  ...
  struct matrix * gen_torus;
  gen_torus = generate_torus(cx, cy, cz, r1, r2, step);
  int i;
  for(i = 0; i < gen_torus->lastcol; i++){
    add_edge(edges, gen_torus->m[0][i],gen_torus->m[1][i],gen_torus->m[2][i],gen_torus->m[0][i],gen_torus->m[1][i],gen_torus->m[2][i]);
    //gen_sphere->m[0][i+1],gen_sphere->m[1][i+1],gen_sphere->m[2][i+1]);
  }
}
